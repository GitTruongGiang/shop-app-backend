...
const fakerShop = async () => {
  ...
  data = new Set(data.filter((e) => e.brand === "Lenovo")); //lenovo, apple, asus, dell, acer
  data = Array.from(data);
  const id = "64059562c152fe177f26b00e";
  ...
  data.forEach(async (e) => {
    await ModelLaptop.create({
      ...
      imageUrl: `https://shop-app-backend-production.up.railway.app/image/lenovo${Math.floor(
        Math.random() * (4 - 1) + 1
      )}.jpg`,
    });
  });
